...
- **İşlemler:**
  ...
  - **Müşteriye Verilen Sabit Toplam Fiyat & Otomatik İşçilik Dengeleme:** "Tamir Ücreti Belirleme" kutusuna müşteriye verilen hepsi dahil toplam fiyat girilir (örn: 750 ₺). Yedek parça ve fason onarım (dış servis) maliyetleri bu tutarın içindedir; sistem kalan tutarı otomatik olarak net işçilik (`Cihaz Onarım & Servis Bedeli`) olarak hesaplar (`Netİşçilik = Toplam - Parçalar - DışServis - DiğerHizmetler`). Parça eklendikçe/çıkarıldıkça veya fason maliyeti değiştikçe genel toplam bozulmadan işçilik tutarı canlı dengelenir.
  - **Depodan Hızlı & Filtrelenebilir Yedek Parça Arama:** Yüzlerce ürün arasından seçim yapmayı kolaylaştırmak için anlık isim, model, SKU ve kategori aramalı akıllı combobox; stok adedi rozeti (`📦 X Adet` veya `Tükendi`) ve dövizden TRY çevrimli birim fiyat ile tek tıkla parça seçimi.
  - **Esnek Teslimat & Tahsilat Tarihi:** Cihaz teslim edilirken veya daha önce teslim edilmiş bir cihaz sisteme girilirken `Teslim Tarihi` serbestçe seçilebilir (örn: dün). Seçilen tarih hem `technical_service_tickets.delivered_at`'e hem de oluşturulan/güncellenen `cash_transactions`, `bank_transactions`, `customer_transactions` ve `stock_transactions` kayıtlarına `tx_date` olarak işlenir; böylece geçmişte teslim edilen cihazlar bugünün değil ilgili günün Mağaza (POS) ekranında gösterilir.
